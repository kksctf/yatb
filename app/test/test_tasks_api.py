...
def test_task_create(client: TestClient):
    ...
    resp1 = client.post(app.url_path_for('api_admin_task_create'),
                        data=schema.TaskForm(task_name="TestTast1",
                                             category="web",
                                             scoring=schema.StaticScoring(static_points=1337),
                                             description="test_task_decription",
                                             flag="kks{test_task}").json(), headers={"Content-Type": "application/json"})
    assert resp1.status_code == 200, resp1.text

    resp2 = client.post(app.url_path_for('api_admin_task_create'),
                        data=schema.TaskForm(task_name="TestTast2",
                                             category="pwn",
                                             scoring=schema.StaticScoring(static_points=1338),
                                             description="second_test_task_description",
                                             flag="kks{other_test_task}").json(), headers={"Content-Type": "application/json"})
    ...
    for task_id, task in resp3.json().items():
        ...
        resp_show = client.post(app.url_path_for('api_admin_task_edit', task_id=task_obj.task_id),
                                data=task_obj.json(), headers={"Content-Type": "application/json"})
